...
def wipe_data(request):
    """Admin-only view to remove all main app data (destructive).

    Requires the user to type DELETE in the confirmation box and submit a POST.
    Only staff or superusers can perform the action.
    """
    ...
    if request.method == 'POST':
        ...
        if confirm == 'DELETE':
            # Delete dependent objects in an order that avoids FK constraint issues
            # Sections reference many other models so delete them first
            ...
            result = 'All SchedulerApp data has been deleted.'
            messages.success(request, 'All SchedulerApp data has been deleted.')
        else:
            result = 'Confirmation failed. Type DELETE to confirm.'

    return render(request, 'wipe_confirm.html', {'result': result})
